...
public final class HighLevelEncoder {
  ...
  public com.google.zxing.common.BitArray encode() {
    Collection<State> states = Collections.singletonList(com.google.zxing.aztec.encoder.State.INITIAL_STATE);
    ...
    com.google.zxing.aztec.encoder.State minState = Collections.min(states, new Comparator<State>() {
      @Override
      public int compare(com.google.zxing.aztec.encoder.State a, com.google.zxing.aztec.encoder.State b) {
  ...
  private Collection<State> updateStateListForChar(Iterable<State> states, int index) {
    ...
    for (com.google.zxing.aztec.encoder.State state : states) {
  ...
  // the "result" list.
  private void updateStateForChar(com.google.zxing.aztec.encoder.State state, int index, Collection<State> result) {
    char ch = (char) (text[index] & 0xFF);
    boolean charInCurrentTable = CHAR_MAP[state.getMode()][ch] > 0;
    com.google.zxing.aztec.encoder.State stateNoBinary = null;
    for (int mode = 0; mode <= MODE_PUNCT; mode++) {
      ...
      if (charInMode > 0) {
        ...
        if (!charInCurrentTable || mode == state.getMode() || mode == MODE_DIGIT) {
          ...
          com.google.zxing.aztec.encoder.State latchState = stateNoBinary.latchAndAppend(mode, charInMode);
        ...
        if (!charInCurrentTable && SHIFT_TABLE[state.getMode()][mode] >= 0) {
          ...
          com.google.zxing.aztec.encoder.State shiftState = stateNoBinary.shiftAndAppend(mode, charInMode);
    ...
    if (state.getBinaryShiftByteCount() > 0 || CHAR_MAP[state.getMode()][ch] == 0) {
      ...
      com.google.zxing.aztec.encoder.State binaryState = state.addBinaryShiftChar(index);
  ...
  private static void updateStateForPair(com.google.zxing.aztec.encoder.State state, int index, int pairCode, Collection<State> result) {
    com.google.zxing.aztec.encoder.State stateNoBinary = state.endBinaryShift(index);
    ...
    if (pairCode == 3 || pairCode == 4) {
      ...
      com.google.zxing.aztec.encoder.State digitState = stateNoBinary
    ...
    if (state.getBinaryShiftByteCount() > 0) {
      ...
      // in binary mode.
      com.google.zxing.aztec.encoder.State binaryState = state.addBinaryShiftChar(index).addBinaryShiftChar(index + 1);
      result.add(binaryState);
    }
  }
  ...
  private static Collection<State> simplifyStates(Iterable<State> states) {
    ...
    for (com.google.zxing.aztec.encoder.State newState : states) {
      boolean add = true;
      for (Iterator<State> iterator = result.iterator(); iterator.hasNext();) {
        com.google.zxing.aztec.encoder.State oldState = iterator.next();
