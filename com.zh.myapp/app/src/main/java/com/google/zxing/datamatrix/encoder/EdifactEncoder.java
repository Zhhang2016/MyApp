...
final class EdifactEncoder implements com.google.zxing.datamatrix.encoder.Encoder {
   ...
   */
  private static void handleEOD(com.google.zxing.datamatrix.encoder.EncoderContext context, CharSequence buffer) {
    try {
      int count = buffer.length();
      if (count == 0) {
      ...
      if (restChars <= 2) {
        ...
        if (available >= 3) {
          ...
          //available = activity.symbolInfo.dataCapacity - activity.getCodewordCount();
        }
      }
