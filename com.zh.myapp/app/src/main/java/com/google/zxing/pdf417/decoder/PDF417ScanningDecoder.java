...
public final class PDF417ScanningDecoder {
  ...
  public static DecoderResult decode(BitMatrix image,
    ...
    for (int barcodeColumnCount = 1; barcodeColumnCount <= maxBarcodeColumn; barcodeColumnCount++) {
      ...
      if (barcodeColumn == 0 || barcodeColumn == maxBarcodeColumn) {
        ...
        detectionResultColumn = new com.google.zxing.pdf417.decoder.DetectionResultColumn(boundingBox);
      }
      ...
      for (int imageRow = boundingBox.getMinY(); imageRow <= boundingBox.getMaxY(); imageRow++) {
        ...
        com.google.zxing.pdf417.decoder.Codeword codeword = detectCodeword(image, boundingBox.getMinX(), boundingBox.getMaxX(), leftToRight,
  ...
  private static DetectionResult merge(DetectionResultRowIndicatorColumn leftRowIndicatorColumn,
    ...
    com.google.zxing.pdf417.decoder.BarcodeMetadata barcodeMetadata = getBarcodeMetadata(leftRowIndicatorColumn, rightRowIndicatorColumn);
  ...
  private static BoundingBox adjustBoundingBox(DetectionResultRowIndicatorColumn rowIndicatorColumn)
    ...
    com.google.zxing.pdf417.decoder.Codeword[] codewords = rowIndicatorColumn.getCodewords();
  ...
  private static com.google.zxing.pdf417.decoder.BarcodeMetadata getBarcodeMetadata(DetectionResultRowIndicatorColumn leftRowIndicatorColumn,
                                                    DetectionResultRowIndicatorColumn rightRowIndicatorColumn) {
  ...
  private static DetectionResultRowIndicatorColumn getRowIndicatorColumn(BitMatrix image,
    ...
    for (int i = 0; i < 2; i++) {
      ...
      for (int imageRow = (int) startPoint.getY(); imageRow <= boundingBox.getMaxY() &&
        ...
        com.google.zxing.pdf417.decoder.Codeword codeword = detectCodeword(image, 0, image.getWidth(), leftToRight, startColumn, imageRow,
  ...
  private static void adjustCodewordCount(DetectionResult detectionResult, com.google.zxing.pdf417.decoder.BarcodeValue[][] barcodeMatrix)
      throws NotFoundException {
  ...
  private static DecoderResult createDecoderResult(DetectionResult detectionResult) throws FormatException,
    ...
    com.google.zxing.pdf417.decoder.BarcodeValue[][] barcodeMatrix = createBarcodeMatrix(detectionResult);
  ...
  private static com.google.zxing.pdf417.decoder.BarcodeValue[][] createBarcodeMatrix(DetectionResult detectionResult) {
    com.google.zxing.pdf417.decoder.BarcodeValue[][] barcodeMatrix =
        new com.google.zxing.pdf417.decoder.BarcodeValue[detectionResult.getBarcodeRowCount()][detectionResult.getBarcodeColumnCount() + 2];
    for (int row = 0; row < barcodeMatrix.length; row++) {
      for (int column = 0; column < barcodeMatrix[row].length; column++) {
        barcodeMatrix[row][column] = new com.google.zxing.pdf417.decoder.BarcodeValue();
      }
    }
  ...
  private static int getStartColumn(DetectionResult detectionResult,
    ...
    com.google.zxing.pdf417.decoder.Codeword codeword = null;
    ...
    while (isValidBarcodeColumn(detectionResult, barcodeColumn - offset)) {
      ...
      for (com.google.zxing.pdf417.decoder.Codeword previousRowCodeword : detectionResult.getDetectionResultColumn(barcodeColumn).getCodewords()) {
  ...
  private static com.google.zxing.pdf417.decoder.Codeword detectCodeword(BitMatrix image,
                                         int minColumn,
                                         int maxColumn,
                                         boolean leftToRight,
                                         int startColumn,
    ...
    int decodedValue = com.google.zxing.pdf417.decoder.PDF417CodewordDecoder.getDecodedValue(moduleBitCount);
    int codeword = PDF417Common.getCodeword(decodedValue);
    if (codeword == -1) {
      return null;
    }
    return new com.google.zxing.pdf417.decoder.Codeword(startColumn, endColumn, getCodewordBucketNumber(decodedValue), codeword);
  }
  ...
  private static DecoderResult decodeCodewords(int[] codewords, int ecLevel, int[] erasures) throws FormatException,
    ...
    DecoderResult decoderResult = com.google.zxing.pdf417.decoder.DecodedBitStreamParser.decode(codewords, String.valueOf(ecLevel));
  ...
  public static String toString(com.google.zxing.pdf417.decoder.BarcodeValue[][] barcodeMatrix) {
    ...
    for (int row = 0; row < barcodeMatrix.length; row++) {
      ...
      for (int column = 0; column < barcodeMatrix[row].length; column++) {
        com.google.zxing.pdf417.decoder.BarcodeValue barcodeValue = barcodeMatrix[row][column];
